...
if (typeof window !== "undefined") {
  isSafari = window.webkitOfflineAudioContext;
}
...
export function getMonoAudio(audioBuffer) {
...
export async function resampleAndMakeMono(audioBuffer, targetSr = SAMPLE_RATE) {
  if (
    ...
    audioBuffer.numberOfChannels === undefined
  )
    return;
...
export function padReflect(data, padding) {
...
export function pow(arr, power) {
  return arr.map((v) => Math.pow(v, power));
}

export function magSpectrogram(stft, power) {
...
export function calculateFftFreqs(sampleRate, nFft) {
  return linearSpace(0, sampleRate / 2, Math.floor(1 + nFft / 2));
}

export function calculateMelFreqs(nMels, fMin, fMax) {
...
export function internalDiff(arr) {
...
export function outerSubtract(arr, arr2) {
  const out = [];
  for (let i = 0; i < arr.length; i++) {
...
export function applyFilterbank(mags, filterbank) {
...
export function applyWholeFilterbank(spec, filterbank) {
